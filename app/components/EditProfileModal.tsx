...
const EditProfileModal: React.FC<EditProfileModalProps> = ({
  ...
  const checkUsernameChangeability = async (newUsername: string) => {
    ...
    if (!profileData) {
      setUsernameError('Profil non chargé');
      return false;
    }

    const history = (profileData.username_change_history as number[]) || [];
    if (!profileService.canChangeUsername(history)) {
      const nextDate = profileService.getNextUsernameChangeDate(history);
      if (nextDate) {
    ...
    try {
      const isAvailable = await profileService.checkUsernameAvailable(newUsername, user?.id);
      if (!isAvailable) {
  ...
  const handleSave = async () => {
    ...
    const canChange = await checkUsernameChangeability(username);
    if (!canChange) {
      return;
    }
    ...
    try {
      ...
      if (avatarFile) {
        ...
        try {
          const updatedProfile = await profileService.uploadAvatar(user.id, avatarFile);
      ...
      if (username !== initialUsername) {
        try {
          await profileService.updateUsername(user.id, username);
          updates.username = username;
          hasChanges = true;
          toast.success('Pseudo mis à jour');
        } catch (error: any) {
          setUsernameError(error.message || 'Erreur lors de la mise à jour du pseudo');
          setIsSaving(false);
          return;
        }
      }
      ...
      if (newAvatarUrl !== (profileData.avatar_url || '')) {
  ...
  const history = (profileData.username_change_history as number[]) || [];
  const canChange = profileService.canChangeUsername(history);
  const nextChangeDate = profileService.getNextUsernameChangeDate(history);
